...
def _apply_pitch_shift(input_path: Path, output_path: Path, pitch: float):
    if not pydub_available:
        raise HTTPException(status_code=501, detail="Audio processing library (pydub) is not installed.")
    try:
        sound = AudioSegment.from_file(input_path)
        
        octaves = (pitch - 1.0) * 1.0 # Simple linear mapping for now
        new_sample_rate = int(sound.frame_rate * (2.0 ** octaves))
        
        hipitch_sound = sound._spawn(sound.raw_data, overrides={'frame_rate': new_sample_rate})
        hipitch_sound = hipitch_sound.set_frame_rate(sound.frame_rate)
        
        hipitch_sound.export(output_path, format="wav")
    except Exception as e:
        trace_exception(e)
        raise HTTPException(status_code=500, detail=f"Failed to apply pitch shift: {e}")
...
async def upload_voice(
    ...
    # Apply pitch shift and save as the final WAV file
    final_filename = f"{uuid.uuid4().hex}.wav"
    final_path = user_voices_path / final_filename
    _apply_pitch_shift(temp_original_path, final_path, pitch)
...
async def update_voice(
    ...
    if file: # If a new file is uploaded
        ...
        old_file_path = user_voices_path / voice_to_update.file_path
        ...
        final_path = user_voices_path / voice_to_update.file_path # Overwrite existing file path
        _apply_pitch_shift(temp_original_path, final_path, pitch)
        temp_original_path.unlink()

    elif voice_to_update.pitch != pitch: # If only pitch changed, reprocess the existing file
        # We need the original, un-pitched file to do this correctly.
        # This simple implementation re-processes from the last processed file, which is not ideal.
        # A more robust solution would store the original upload separately.
        current_file_path = user_voices_path / voice_to_update.file_path
        if current_file_path.exists():
            _apply_pitch_shift(current_file_path, current_file_path, pitch)
        else:
            raise HTTPException(status_code=404, detail="Original voice file not found for pitch adjustment. Please re-upload.")
...
async def set_active_voice(
    ...
    # We need to reconstruct the full UserAuthDetails model.
    # For now, we will return a partial update. A full refresh on the client is better.
...
async def test_voice(
    ...
    try:
        audio_bytes = lc.tts.generate_audio(text=request.text, voice=str(voice_file_path.resolve()))
        return FileResponse(path=io.BytesIO(audio_bytes), media_type="audio/wav", filename="test_output.wav")
    except Exception as e:
        trace_exception(e)
