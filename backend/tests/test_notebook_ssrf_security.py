...
class TestNotebookIngestionSSRFSecurity(unittest.TestCase):
    """
    Regression test for the SSRF vulnerability in the notebook ingestion task.
    Ensures that internal IP addresses and cloud metadata endpoints are blocked
    before issuing outbound requests.get() calls for Arxiv PDF ingestion.
    """
    ...
    @patch('requests.get')
    def test_ingestion_task_blocks_ssrf(self, mock_get):
