...
class SharedDiscussionLink(Base):
    ...
    __table_args__ = (
        UniqueConstraint('discussion_id', 'owner_user_id', 'shared_with_user_id', name='uq_discussion_owner_shared_user'),
        CheckConstraint(permission_level.in_(['view', 'interact']), name='ck_permission_level_valid')
    )