...
export const useNotebookStore = defineStore('notebooks', () => {
    ...
    async function selectNotebook(id) {
        ...
        try {
            const res = await apiClient.get('/api/notebooks');
            const updated = res.data.find(n => n.id === id);
            if (updated) {
    ...
    async function saveActive() {
        ...
        try {
            ...
            const idx = notebooks.value.findIndex(n => n.id === updated.id);
    ...
    async function createManualArtefact(title, content) {
        if (!activeNotebook.value) return;
        try {
            await apiClient.post(`/api/notebooks/${activeNotebook.value.id}/artefact`, { title, content });
            await selectNotebook(activeNotebook.value.id);
    ...
    async function sendSlideMessage(tabId, slideId, prompt, selectedArtefacts = []) {
        if (!activeNotebook.value) return;
        try {
            const res = await apiClient.post(`/api/notebooks/${activeNotebook.value.id}/slide_chat`, {
    ...
    async function brainstormSlide(topic, layout, selectedArtefacts = [], author = '') {
        if (!activeNotebook.value) return;
        try {
            const res = await apiClient.post(`/api/notebooks/${activeNotebook.value.id}/brainstorm`, {
    ...
    async function generateSummary() {
        if (!activeNotebook.value) return;
        try {
            const res = await apiClient.post(`/api/notebooks/${activeNotebook.value.id}/generate_summary`);
