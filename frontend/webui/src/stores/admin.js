...
export const useAdminStore = defineStore('admin', () => {
    ...
    async function sanitizeDatabase() {
        const response = await apiClient.post('/api/admin/security/sanitize-content');
        tasksStore.addTask(response.data);
    }
    ...
    async function installTtiFromZoo(id, index) { const r = await apiClient.post(`/api/admin/tti-bindings/${id}/zoo/install`, { index }); tasksStore.addTask(r.data); return r.data; }
    async function fetchTtsBindingZoo(id) { const r = await apiClient.get(`/api/admin/tts-bindings/${id}/zoo`); return r.data; }
    ...
    async function restartApp(appId) { const res = await apiClient.post(`/api/apps_zoo/installed/${appId}/restart`); tasksStore.addTask(res.data); }
    async function updateApp(appId) { const res = await apiClient.post(`/api/apps_zoo/installed/${appId}/update`); tasksStore.addTask(res.data); }
    async function uninstallApp(appId) { await apiClient.delete(`/api/apps_zoo/installed/${appId}`); await fetchZooApps(true); await fetchZooMcps(true); uiStore.addNotification('Item uninstalled.', 'success'); }
    async function updateInstalledApp(appId, payload) { await apiClient.put(`/api/apps_zoo/installed/${appId}`, payload); await fetchZooApps(true); await fetchZooMcps(true); }
    ...
    async function deleteRegisteredApp(appId) { await apiClient.delete(`/api/apps/${appId}`); await fetchZooApps(true); uiStore.addNotification('App registration removed.', 'success'); }
    async function deleteRegisteredMcp(mcpId) { await apiClient.delete(`/api/mcps/${mcpId}`); await fetchZooMcps(true); uiStore.addNotification('MCP registration removed.', 'success'); }
    ...
    async function createFunFactCategory(payload) { await apiClient.post('/api/admin/fun-facts/categories', payload); await fetchFunFactCategories(true); }
    async function updateFunFactCategory(id, payload) { await apiClient.put(`/api/admin/fun-facts/categories/${id}`, payload); await fetchFunFactCategories(true); }
    async function deleteFunFactCategory(id) { await apiClient.delete(`/api/admin/fun-facts/categories/${id}`); await Promise.all([fetchFunFactCategories(true), fetchFunFacts(true)]); }
    ...
    return {
        ...
        serviceStats, isLoadingServiceStats, 
