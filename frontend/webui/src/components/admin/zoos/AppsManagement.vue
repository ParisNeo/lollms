...
function selectCategory(cat) {
    appFilters.selectedCategory = cat;
    appFilters.currentPage = 1;
    adminStore.fetchZooApps();
}

function filterByRunning() {
    if (appFilters.installationStatusFilter === 'Running') {
        appFilters.installationStatusFilter = 'All';
    } else {
        appFilters.installationStatusFilter = 'Running';
    }
    appFilters.currentPage = 1;
    adminStore.fetchZooApps();
}
...
const itemsWithTaskStatus = computed(() => {
    ...
    return (zooApps.value.items || []).map(item => ({
        ...item, 
        task: taskMap.get(`folder:${item.folder_name}`) || (item.id ? taskMap.get(`id:${item.id}`) : null)
    }));
});

// Live Running Services Computer
const runningApps = computed(() => {
    return (zooApps.value.items || []).filter(item => item.is_installed && item.status === 'running');
});

const installedAppsCount = computed(() => {
    return (zooApps.value.items || []).filter(item => item.is_installed).length;
});
...
async function showItemHelp(item) { 
    const readme = await adminStore.fetchAppReadme(item.repository, item.folder_name); 
...
<template>
    <div class="space-y-6">
        ...
        <section v-if="activeSubTab === 'zoo'" class="space-y-5">
                ...
                <div>
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        ...
                        <button 
                            ...
                            :class="appFilters.installationStatusFilter === 'Running' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300 hover:bg-emerald-200'"
                            title="Click to toggle filtering for running apps only"
                        >
                            <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                            <span>{{ runningApps.length }} Running</span>
                        </button>
                    </h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Explore, launch, and manage modular web apps with live port tracking.</p>
                </div>

                <div class="flex items-center gap-2 w-full sm:w-auto">
                    <button @click="handleRegisterApp" class="btn btn-secondary btn-sm flex-1 sm:flex-none justify-center">
                        <IconPlus class="w-3.5 h-3.5 mr-1" />Register App
                    </button>
            ...
            <!-- Running Apps Quick Access Bar -->
            <div v-if="runningApps.length > 0" class="p-3 bg-emerald-50/70 dark:bg-emerald-950/20 border border-emerald-200/80 dark:border-emerald-800/40 rounded-2xl">
                <div class="flex items-center justify-between gap-2 mb-2 px-1">
                    <div class="flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                        <span class="text-xs font-bold text-emerald-900 dark:text-emerald-300 uppercase tracking-wider">Active Services & Port Allocations</span>
                    </div>
                    <span class="text-[11px] font-medium text-emerald-700 dark:text-emerald-400 font-mono">
                        {{ runningApps.length }} port{{ runningApps.length > 1 ? 's' : '' }} assigned
                    </span>
                </div>
                ...
                <div class="flex items-center gap-2 overflow-x-auto py-1 custom-scrollbar">
                    ...
                    >
                        ...
                        <span class="text-xs font-bold text-gray-900 dark:text-white max-w-[120px] truncate" :title="app.name">{{ app.name }}</span>
                        ...
                        <div class="flex items-center gap-1 pl-1 border-l border-gray-200 dark:border-gray-700">
                            ...
                            <button 
                                @click="handleAppAction(app.id, 'stop')" 
            ...
            <div class="space-y-3">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                    ...
                    <div class="relative lg:col-span-2">
                        <input 
                            type="text" 
                            v-model="appFilters.searchQuery" 
                            placeholder="Search applications by name, author, or description..." 
                    ...
                    <!-- Status Filter -->
                    <select v-model="appFilters.installationStatusFilter" class="input-field text-xs py-2 rounded-xl font-medium">
                    ...
                    <!-- Repository Filter -->
                    <select v-model="appFilters.selectedRepository" class="input-field text-xs py-2 rounded-xl">
                    ...
                    <div class="flex items-center gap-1.5">
                        <select v-model="appFilters.sortKey" class="input-field text-xs py-2 rounded-xl grow">
                            <option v-for="opt in sortOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
                        </select>
                        <button 
                            @click="appFilters.sortOrder = appFilters.sortOrder === 'asc' ? 'desc' : 'asc'" 
                            class="btn btn-secondary p-2 rounded-xl shrink-0"
                            :title="appFilters.sortOrder === 'asc' ? 'Sort Ascending' : 'Sort Descending'"
                        >
                            <IconArrowUp v-if="appFilters.sortOrder === 'asc'" class="w-4 h-4" />
                ...
                <div class="flex items-center gap-1.5 overflow-x-auto py-1 custom-scrollbar">
                    <button 
                        ...
                        :class="appFilters.selectedCategory === cat ? 'bg-blue-600 text-white shadow-xs' : 'bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300'"
                    >
            ...
            <!-- Apps Grid Area -->
            <div v-if="isLoadingZooApps" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5">
            ...
            <div v-else-if="!itemsWithTaskStatus || itemsWithTaskStatus.length === 0" class="text-center py-16 px-4 rounded-2xl bg-white/50 dark:bg-gray-800/40 border border-gray-200/80 dark:border-gray-700/60">
                ...
                <h4 class="font-bold text-sm text-gray-800 dark:text-gray-200">No Apps Found</h4>
                <p class="text-xs text-gray-500 mt-1 max-w-sm mx-auto">No applications match your active query or filters.</p>
                <button 
                    v-if="appFilters.searchQuery || appFilters.selectedCategory !== 'All' || appFilters.installationStatusFilter !== 'All'"
                    @click="appFilters.searchQuery = ''; appFilters.selectedCategory = 'All'; appFilters.installationStatusFilter = 'All'; adminStore.fetchZooApps();"
        ...
        <section v-if="activeSubTab === 'source'" class="space-y-5">
            <div class="flex justify-between items-center bg-white/60 dark:bg-gray-850/50 p-4 rounded-2xl border border-gray-200/80 dark:border-gray-700/60 backdrop-blur-md">
                <div>
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">App Zoo Repositories</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Manage Git repositories and local directories serving application definitions.</p>
                </div>
            ...
            <div v-if="isAddRepoFormVisible" class="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
                <form @submit.prevent="handleAddRepository" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input v-model="newRepo.name" type="text" class="input-field" placeholder="e.g. My Custom Apps Zoo" required>
                    </div>
                    ...
                    <div v-if="newRepo.type === 'git'">
                        ...
                        <input v-model="newRepo.url" type="url" class="input-field" placeholder="https://github.com/user/repo.git" :required="newRepo.type === 'git'">
                    </div>
                    ...
                    <div v-if="newRepo.type === 'local'">
                        ...
                        <input v-model="newRepo.path" type="text" class="input-field" placeholder="/absolute/path/to/zoo" :required="newRepo.type === 'local'">
                    </div>
            ...
            <!-- Repositories List -->
            <div v-if="isLoadingZooRepositories" class="text-center py-8 text-sm text-gray-500">Loading repositories...</div>
            <div v-else-if="!sortedRepositories || sortedRepositories.length === 0" class="text-center py-12 rounded-2xl bg-gray-50 dark:bg-gray-800/50 text-gray-500">
                <p class="text-sm">No external repositories configured.</p>
            </div>
