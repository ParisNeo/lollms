<!-- frontend/webui/src/components/admin/zoos/McpsManagement.vue -->
<script setup>
...
async function handleRestartApp(app) {
    if (await uiStore.showConfirmation({ title: `Restart '${app.name}'?`, message: 'The MCP will be stopped and started again.', confirmText: 'Restart' })) {
...
async function handlePurgeItem(item) { if (await uiStore.showConfirmation({ title: `Purge '${item.name}'?`, message: 'This will permanently delete the installation folder.', confirmText: 'Purge' })) { await adminStore.purgeBrokenInstallation({...item, item_type: 'mcp'}); }}
...
<template>
    <div class="space-y-6">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 lg:col-span-3 gap-4">
                        <select v-model="installationStatusFilter" class="input-field"><option value="All">All Statuses</option><option value="Installed">Installed</option><option value="Uninstalled">Uninstalled</option><option value="Registered">Registered</option><option value="Broken">Broken</option></select>
                        <select v-model="selectedCategory" class="input-field"><option v-for="cat in categories" :key="cat" :value="cat">{{ cat }}</option></select>
                        <select v-model="selectedRepository" class="input-field"><option value="All">All Sources</option><option value="Registered">Registered</option><option v-for="repo in sortedRepositories" :key="repo.id" :value="repo.name">{{ repo.name }}</option></select>
                    </div>
                </div>
        ...
        <section v-if="activeSubTab === 'source'">
            <div class="flex justify-between items-center mb-4 flex-wrap gap-2"><h3 class="text-xl font-semibold">MCP Zoo Repositories</h3><button @click="isAddRepoFormVisible = !isAddRepoFormVisible" class="btn btn-primary">{{ isAddRepoFormVisible ? 'Cancel' : 'Add Repository' }}</button></div>
