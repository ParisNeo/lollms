...
const selectedBindingType = computed(() => {
    ...
    return availableTtiBindingTypes.value.find(b => b.binding_name === form.value.name);
});
...
function showEditForm(binding) {
    ...
    const bindingType = availableTtiBindingTypes.value.find(b => b.binding_name === binding.name);
...
function manageModels(binding) {
    uiStore.openModal('manageModels', { binding, bindingType: 'tti' });
}

function getBindingTitle(name) {
    const bindingType = availableTtiBindingTypes.value.find(b => b.binding_name === name);
...
async function executeCommand(cmd, bindingId, params) {
    try {
        ...
        const result = await adminStore.executeTtiBindingCommand(bindingId, cmd.name, params);
        if (result) {
...
<template>
    <div class="space-y-8">
        <div v-if="isFormVisible" class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            <h3 class="text-xl font-semibold mb-4">{{ isEditMode ? 'Edit TTI Binding' : 'Add New TTI Binding' }}</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="name" v-model="form.name" class="input-field mt-1" required :disabled="isEditMode">
                            ...
                            <option v-for="type in availableTtiBindingTypes" :key="type.binding_name" :value="type.binding_name">{{ type.title }}</option>
                        </select>
                    </div>
                </div>
        ...
        <div>
            ...
            <div v-else class="space-y-4">
                <div v-for="binding in ttiBindings" :key="binding.id" class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md flex flex-col gap-4">
                    <div class="flex-grow">
                    ...
                    <div class="border-t dark:border-gray-700 pt-3 flex justify-end gap-2">
                        <button v-if="availableTtiBindingTypes.find(b => b.binding_name === binding.name)?.commands?.length" @click="showCommands(binding)" class="btn btn-secondary btn-sm flex items-center gap-2" title="Execute Commands">
        ...
        <!-- Commands Modal -->
        <GenericModal :visible="isCommandsModalVisible" title="TTI Binding Commands" @close="isCommandsModalVisible = false">
            <div class="space-y-4">
