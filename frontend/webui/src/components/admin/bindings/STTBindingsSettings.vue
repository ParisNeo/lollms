...
const localSttModelDisplayMode = ref('mixed');
...
async function checkZooAvailability(bindingId) {
    ...
    hasZoo.value = false;
    try {
        const res = await adminStore.fetchSttBindingZoo(bindingId);
...
async function handleHealProfiles() {
    try {
        ...
        await adminStore.fetchSttBindings(true);
    } catch (e) {
...
watch(globalSettings, (newSettings) => {
    if (Array.isArray(newSettings)) {
        const setting = newSettings.find(s => s.key === 'stt_model_display_mode');
        if (setting) {
            localSttModelDisplayMode.value = setting.value;
        }
    }
...
function handleDisplayModeChange(event) {
    ...
    localSttModelDisplayMode.value = newValue;
    adminStore.updateGlobalSettings({ 'stt_model_display_mode': newValue });
}
...
async function handleSubmit() {
    ...
    try {
        ...
        if (isEditMode.value) {
            await adminStore.updateSttBinding(editingBinding.value.id, payload);
        } else {
            await adminStore.addSttBinding(payload);
        }
        hideForm();
    } catch (error) {
...
async function handleDelete(binding) {
    const confirmed = await uiStore.showConfirmation({
        title: `Delete STT Binding '${binding.alias}'?`,
        message: 'Are you sure? This action cannot be undone.',
        confirmText: 'Delete'
    });
    if (confirmed) {
        await adminStore.deleteSttBinding(binding.id);
    }
}
    ...
    <div class="space-y-8">
        <div v-if="isFormVisible" class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                ...
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" id="alias" v-model="form.alias" class="input-field mt-1" required placeholder="e.g., local_whisper" autocomplete="off">
                    ...
                    <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-3 rounded-md">
                        <span class="grow flex flex-col"><span class="text-sm font-medium text-gray-900 dark:text-gray-100">Active</span><span class="text-sm text-gray-500 dark:text-gray-400">If disabled, this STT service will not be available.</span></span>
            ...
            <div v-else-if="activeTab === 'models'">
                <BindingModelsManager :binding="editingBinding" binding-type="stt" />
                <div class="flex justify-end gap-3 mt-4">
                     <button type="button" @click="hideForm" class="btn btn-secondary">Close</button>
                </div>
            </div>
            ...
            <div class="flex justify-between items-center flex-wrap gap-4 bg-white/60 dark:bg-gray-850/50 p-4 rounded-2xl border border-gray-200/80 dark:border-gray-700/60 backdrop-blur-md">
                <div>
                    <h2 class="text-xl font-black tracking-tight text-gray-900 dark:text-white flex items-center gap-2">
                        <IconMicrophone class="w-6 h-6 text-cyan-500" />
                        <span>STT Transcription Bindings</span>
                        <span class="text-xs font-bold px-2.5 py-0.5 rounded-full bg-cyan-50 text-cyan-600 dark:bg-cyan-900/40 dark:text-cyan-300">
                            {{ sttBindings.length }} Connections
                        </span>
                    </h2>
                    <p class="text-xs text-gray-500 mt-0.5">Manage speech-to-text engines and audio transcription models.</p>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button @click="openPolicyModal" class="btn btn-secondary btn-sm flex items-center gap-1.5" title="Force models or set system defaults for users">
                        <IconCpuChip class="w-3.5 h-3.5 text-cyan-500" />
                        <span>⚡ Policy & Defaults</span>
                    </button>

                    <button @click="showAddForm" class="btn btn-primary btn-sm flex items-center gap-1.5 shadow-sm">
                        <span>+ Add STT Binding</span>
                    </button>
                </div>
            </div>

            <div v-if="isLoadingSttBindings" class="text-center p-6">Loading STT bindings...</div>
            <div v-else-if="sttBindings.length === 0 && !isFormVisible" class="text-center p-6 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                <p>No STT bindings configured yet.</p>
                <button @click="showAddForm" class="mt-2 text-blue-600 hover:underline">Add your first one</button>
            </div>
            <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                <div v-for="binding in sttBindings" :key="binding.id" @click="showEditForm(binding)" class="bg-white dark:bg-gray-800 p-5 rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer flex flex-col gap-4 border border-transparent hover:border-blue-500 group relative">
                    ...
                    <div class="grow">
                         <div class="flex items-center gap-3 mb-2">
                             <IconMicrophone class="w-8 h-8 text-blue-500" />
                             <div>
