...
import IconSpeakerWave from '../../../assets/icons/IconSpeakerWave.vue'; // Music icon
...
async function checkZooAvailability(bindingId) {
    ...
    hasZoo.value = false;
    try {
        const res = await adminStore.fetchTtmBindingZoo(bindingId);
...
async function handleSubmit() {
    ...
    try {
        ...
        if (isEditMode.value) await adminStore.updateTtmBinding(editingBinding.value.id, payload);
        else await adminStore.addTtmBinding(payload);
        hideForm();
    } catch (error) {
...
async function handleDelete(binding) {
    if (await uiStore.showConfirmation({ title: `Delete TTM Binding '${binding.alias}'?`, message: 'Are you sure?', confirmText: 'Delete' })) {
        await adminStore.deleteTtmBinding(binding.id);
    }
}

async function toggleBindingActive(binding) {
    await adminStore.updateTtmBinding(binding.id, { is_active: !binding.is_active });
}
...
async function executeCommand(cmd, bindingId, params) {
    ...
    try {
        ...
        const taskInfo = await adminStore.executeTtmBindingCommand(bindingId, cmd.name, params);
    ...
    <div class="space-y-8">
        <div v-if="isFormVisible" class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-xl font-semibold">{{ isEditMode ? 'Edit TTM Binding: ' + form.alias : 'Add New TTM Binding' }}</h3>
            ...
            <div v-if="activeTab === 'settings'">
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    ...
                    <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-3 rounded-md">
                        <span class="grow flex flex-col"><span class="text-sm font-medium text-gray-900 dark:text-gray-100">Active</span><span class="text-sm text-gray-500 dark:text-gray-400">If disabled, this TTM service will not be available.</span></span>
            ...
            <div v-else-if="activeTab === 'zoo'">
                <BindingZoo :binding="editingBinding" binding-type="ttm" />
                <div class="flex justify-end gap-3 mt-4"><button type="button" @click="hideForm" class="btn btn-secondary">Close</button></div>
            </div>

            <div v-else-if="activeTab === 'models'">
                <BindingModelsManager :binding="editingBinding" binding-type="ttm" />
                <div class="flex justify-end gap-3 mt-4"><button type="button" @click="hideForm" class="btn btn-secondary">Close</button></div>
            </div>
        </div>
        ...
        <div v-else>
            ...
            <div v-if="isLoadingTtmBindings" class="text-center p-6">Loading TTM bindings...</div>
            <div v-else-if="ttmBindings.length === 0" class="text-center p-6 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                <p>No TTM bindings configured yet.</p>
                <button @click="showAddForm" class="mt-2 text-blue-600 hover:underline">Add your first one</button>
            </div>
            <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                <div v-for="binding in ttmBindings" :key="binding.id" @click="showEditForm(binding)" class="bg-white dark:bg-gray-800 p-5 rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer flex flex-col gap-4 border border-transparent hover:border-blue-500 group relative">
                    ...
                    <div class="grow">
                         <div class="flex items-center gap-3 mb-2">
                             <IconSpeakerWave class="w-8 h-8 text-blue-500" />
