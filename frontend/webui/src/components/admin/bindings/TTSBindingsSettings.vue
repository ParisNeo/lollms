...
watch(() => form.value.name, (newName, oldName) => {
    if (newName !== oldName && !isEditMode.value) {
        if (!Array.isArray(availableTtsBindingTypes.value)) return;
...
function showEditForm(binding) {
    ...
    const bindingType = Array.isArray(availableTtsBindingTypes.value) ? availableTtsBindingTypes.value.find(b => (b.binding_name || b.name) === binding.name) : null;
...
async function toggleBindingActive(binding) {
    await adminStore.updateTtsBinding(binding.id, { is_active: !binding.is_active });
}

function getBindingTitle(name) {
    if (!Array.isArray(availableTtsBindingTypes.value)) return name;
...
async function executeCommand(cmd, bindingId, params) {
    ...
    try {
        ...
        const taskInfo = await adminStore.executeTtsBindingCommand(bindingId, cmd.name, params);
    ...
    <div class="space-y-8">
        <div v-if="isFormVisible" class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            ...
            <div v-if="activeTab === 'settings'">
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    ...
                    <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-3 rounded-md">
                        <span class="grow flex flex-col"><span class="text-sm font-medium text-gray-900 dark:text-gray-100">Active</span><span class="text-sm text-gray-500 dark:text-gray-400">If disabled, this TTS service will not be available.</span></span>
             ...
             <div v-else-if="activeTab === 'zoo'">
                <BindingZoo :binding="editingBinding" binding-type="tts" />
                <div class="flex justify-end gap-3 mt-4">
                     <button type="button" @click="hideForm" class="btn btn-secondary">Close</button>
                </div>
            </div>
            ...
            <div v-else-if="activeTab === 'models'">
                <BindingModelsManager :binding="editingBinding" binding-type="tts" />
                <div class="flex justify-end gap-3 mt-4">
                     <button type="button" @click="hideForm" class="btn btn-secondary">Close</button>
                </div>
            </div>
        ...
        <div v-else>
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                <div v-for="binding in ttsBindings" :key="binding.id" @click="showEditForm(binding)" class="bg-white dark:bg-gray-800 p-5 rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer flex flex-col gap-4 border border-transparent hover:border-blue-500 group relative">
