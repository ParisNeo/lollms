...
const { ttvBindings, availableTtvBindingTypes, isLoadingTtvBindings } = storeToRefs(adminStore);
...
onMounted(() => {
    adminStore.fetchTtvBindings();
    adminStore.fetchAvailableTtvBindingTypes();
});
...
function parseOptions(options) {
    ...
    if (Array.isArray(options)) return options.filter(o => o);
    return [];
}
...
async function handleSubmit() {
    ...
    try {
        ...
        if (isEditMode.value) await adminStore.updateTtvBinding(editingBinding.value.id, payload);
        else await adminStore.addTtvBinding(payload);
        hideForm();
    } catch (error) {
...
async function handleDelete(binding) {
    if (await uiStore.showConfirmation({ title: `Delete TTV Binding '${binding.alias}'?`, message: 'Are you sure?', confirmText: 'Delete' })) {
        await adminStore.deleteTtvBinding(binding.id);
    }
}

async function toggleBindingActive(binding) {
    await adminStore.updateTtvBinding(binding.id, { is_active: !binding.is_active });
}
...
async function executeCommand(cmd, bindingId, params) {
    ...
    try {
        ...
        const taskInfo = await adminStore.executeTtvBindingCommand(bindingId, cmd.name, params);
    ...
    <div class="space-y-8">
        <div v-if="isFormVisible" class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                ...
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" id="alias" v-model="form.alias" class="input-field mt-1" required placeholder="e.g., local_video" autocomplete="off">
                        </div>
                        <div>
                        ...
                        <div v-for="param in allFormParameters" :key="param.name" class="space-y-1">
                            ...
                            <div v-else-if="['str', 'int', 'float'].includes(param.type)">
                                <div class="relative">
                                    ...
                                    <button v-if="param.name.includes('key') || param.name.includes('token')" type="button" @click="isKeyVisible[param.name] = !isKeyVisible[param.name]" class="absolute inset-y-0 right-0 px-3 flex items-center text-gray-500 hover:text-gray-700" :title="isKeyVisible[param.name] ? 'Hide' : 'Show'">
                    ...
                    <div class="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-3 rounded-md">
                        <span class="flex-grow flex flex-col"><span class="text-sm font-medium text-gray-900 dark:text-gray-100">Active</span><span class="text-sm text-gray-500 dark:text-gray-400">If disabled, this TTV service will not be available.</span></span>
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit" class="btn btn-primary" :disabled="isLoadingForm">
                            <IconAnimateSpin v-if="isLoadingForm" class="w-5 h-5 mr-2" /> {{ isLoadingForm ? 'Saving...' : (isEditMode ? 'Save Changes' : 'Add Binding') }}
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div v-else-if="activeTab === 'commands'" class="space-y-6">
                ...
                <div class="flex justify-end gap-3 mt-4"><button type="button" @click="hideForm" class="btn btn-secondary">Close</button></div>
            </div>

            <div v-else-if="activeTab === 'zoo'">
                <BindingZoo :binding="editingBinding" binding-type="ttv" />
                <div class="flex justify-end gap-3 mt-4"><button type="button" @click="hideForm" class="btn btn-secondary">Close</button></div>
            </div>

            <div v-else-if="activeTab === 'models'">
                <BindingModelsManager :binding="editingBinding" binding-type="ttv" />
                <div class="flex justify-end gap-3 mt-4"><button type="button" @click="hideForm" class="btn btn-secondary">Close</button></div>
            </div>
        </div>
        ...
        <div v-else>
            ...
            <div v-if="isLoadingTtvBindings" class="text-center p-6">Loading TTV bindings...</div>
            <div v-else-if="ttvBindings.length === 0" class="text-center p-6 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                <p>No TTV bindings configured yet.</p>
                <button @click="showAddForm" class="mt-2 text-blue-600 hover:underline">Add your first one</button>
            </div>
            <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                <div v-for="binding in ttvBindings" :key="binding.id" @click="showEditForm(binding)" class="bg-white dark:bg-gray-800 p-5 rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer flex flex-col gap-4 border border-transparent hover:border-blue-500 group relative">
                    <div class="absolute top-4 right-4 z-10" @click.stop>
                         <button @click="toggleBindingActive(binding)" type="button" :class="[binding.is_active ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-600', 'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out']">
                    ...
                    <div class="flex-grow">
                         <div class="flex items-center gap-3 mb-2">
                             <IconVideoCamera class="w-8 h-8 text-blue-500" />
                             <div><h4 class="font-bold text-lg text-gray-900 dark:text-white">{{ binding.alias }}</h4><p class="text-xs text-gray-500 dark:text-gray-400">{{ getBindingTitle(binding.name) }}</p></div>
                         </div>
                     ...
                     <div class="border-t dark:border-gray-700 pt-3 flex justify-between items-center text-xs text-gray-500"><span>Click to edit</span><button @click.stop="handleDelete(binding)" class="text-red-500 hover:underline p-1">Delete</button></div>
                </div>
            </div>
        </div>
    </div>
