...
async function handleSubmit() {
    ...
    try {
        ...
        if (isEditMode.value) {
            // Only send the key if it has been changed from the placeholder
            if (form.value.service_key === '********') {
                delete payload.service_key;
            }
        ...
        <div v-if="isFormVisible" class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="models_path" v-model="form.models_path" class="input-field mt-1" placeholder="e.g., /path/to/your/models" autocomplete="off">
                 ...
                 <div>
                    ...
                    <div class="relative mt-1">
                        <input :type="isKeyVisible ? 'text' : 'password'" id="service_key" v-model="form.service_key" class="input-field pr-10" :placeholder="isEditMode ? 'Leave blank to keep existing key' : 'Paste API key here'" autocomplete="new-password">
                        <button type="button" @click="isKeyVisible = !isKeyVisible" class="absolute inset-y-0 right-0 px-3 flex items-center text-gray-500 hover:text-gray-700 dark:hover:text-gray-300" :title="isKeyVisible ? 'Hide key' : 'Show key'">
