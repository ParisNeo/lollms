...
const repoMcpCounts = computed(() => {
    if (!zooMcps.value.items) return {};
    return zooMcps.value.items.reduce((acc, mcp) => {
        acc[mcp.repository] = (acc[mcp.repository] || 0) + 1;
        return acc;
    }, {});
});
...
const itemsWithTaskStatus = computed(() => {
    ...
    const taskPrefixes = ['Installing MCP: ', 'Updating app: '];
...
const categories = computed(() => {
    return zooMcps.value.categories || ['All', 'Starred'];
});
...
const installedItemsWithTaskStatus = computed(() => {
    ...
    tasks.value.forEach(task => {
        ...
        if (appName && (task.status === 'running' || task.status === 'pending')) {
            if (!taskMap.has(appName) || new Date(task.created_at) > new Date(taskMap.get(appName).created_at)) {
                taskMap.set(appName, { task, activityType });
            }
        }
    });
    return sorted.map(app => {
        ...
        return { 
            ...
            activity_status: taskInfo ? taskInfo.activityType : null
        }
    });
});
...
async function handleUpdateApp(app) {
    const confirmed = await uiStore.showConfirmation({ 
        ...
        message: 'This will update the MCP to the latest version while preserving your configuration. The MCP will be stopped during the update.',
        confirmText: 'Update'
    });
    if (confirmed) {
    ...
    <div class="space-y-6">
                ...
                <div v-for="repo in sortedRepositories" :key="repo.id" class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div class="flex-grow truncate">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-gray-500">({{ repoMcpCounts[repo.name] || 0 }} items)</span>
                        </div>
        ...
        <section v-if="activeSubTab === 'available_items'">
            ...
            <div class="space-y-4">
                ...
                <div v-else-if="!itemsWithTaskStatus || itemsWithTaskStatus.length === 0" class="empty-state-card"><h4 class="font-semibold">No MCPs Found</h4><p class="text-sm">No MCPs match your criteria. Please add and pull a repository or adjust your filters.</p></div>
                <div v-else>
                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                        <AppCard v-for="item in itemsWithTaskStatus" :key="`${item.repository}/${item.folder_name}`" :app="item" :task="item.task" :is-starred="starredItems.includes(item.name)" item-type-name="MCP" @star="handleStarToggle(item.name)" @install="handleInstallItem(item)" @update="handleUpdateApp(item)" @uninstall="handleUninstallApp(item)" @details="showItemDetails(item)" @help="showItemHelp(item)" @view-task="viewTask" @cancel-install="handleCancelTask(item.task.id)" @start="handleAppAction(item.id, 'start')" @stop="handleAppAction(item.id, 'stop')" />
                    </div>
        ...
        <section v-if="activeSubTab === 'installed_apps'">
             <div class="flex justify-between items-center mb-4">
                <h3 class="text-xl font-semibold">Installed MCPs</h3>
                <div class="flex items-center gap-2">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                <div v-for="app in installedItemsWithTaskStatus" :key="app.id" class="flex flex-col">
                    <AppCard :app="app" :task="app.task" item-type-name="MCP" @update="handleUpdateApp(app)" @uninstall="handleUninstallApp(app)" @start="handleAppAction(app.id, 'start')" @stop="handleAppAction(app.id, 'stop')" />
