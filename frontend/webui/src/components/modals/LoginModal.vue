...
// Use defineAsyncComponent to properly handle lazy-loading components for the tabs.
// This resolves the "[object Promise]" rendering issue.
...
export default {
  ...
  components: {
    GenericModal,
    // The async components do not need to be registered here
    // as they are passed directly to `<component :is="...">`
  },
  data() {
    return {
      activeTab: 'account',
...
<template>
  <GenericModal modalName="settings" title="Settings" maxWidthClass="max-w-7xl">
    ...
    <template #footer>
      ...
      <!-- Save buttons are now within each tab component -->
    </template>
  </GenericModal>
</template>