...
async function handleSubmit() {
    ...
    try {
        await adminStore.sendEmailToUsers(subject.value, body.value, [user.value.id], backgroundColor.value, sendAsText.value);
            ...
            <form @submit.prevent="handleSubmit" class="p-6 space-y-6">
                 <div class="p-3 rounded-md bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600">
                    ...
                    <p v-if="!isUserEligible" class="mt-1 text-xs text-yellow-600 dark:text-yellow-400">
                        Warning: This user may not receive the email because they are inactive or have disabled email notifications.
                    </p>
                </div>
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <label for="email-bg-color-single" class="block text-sm font-medium" :class="{'text-gray-400 dark:text-gray-500': sendAsText}">BG Color</label>
                        <input id="email-bg-color-single" v-model="backgroundColor" type="color" class="w-20 h-10 mt-1 p-1 border rounded-md" :disabled="sendAsText" />
                    </div>
                </div>
                <div>
                ...
                <div class="space-y-4">
                     <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <label for="send-as-text-single" class="text-sm font-medium text-gray-900 dark:text-gray-100">Send as plain text</label>
                     ...
                     <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <label for="show-custom-prompt-single" class="text-sm font-medium text-gray-900 dark:text-gray-100">Use Custom AI Prompt</label>
