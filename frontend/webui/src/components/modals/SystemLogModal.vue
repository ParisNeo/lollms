    ...
    <GenericModal modal-name="systemLog" title="System Logs (Recent Tasks)" max-width-class="max-w-5xl">
        <template #body>
            <div class="flex flex-col h-[70vh]">
                ...
                <div class="grow overflow-y-auto bg-gray-50 dark:bg-gray-900 rounded-md border dark:border-gray-700 p-4 font-mono text-xs">
                    ...
                    <div v-else>
                        <div v-for="(log, idx) in logs" :key="idx" class="mb-1 border-b border-gray-100 dark:border-gray-800 pb-1 last:border-0 hover:bg-gray-100 dark:hover:bg-gray-800/50 px-2 rounded">
                            <div class="flex gap-2">
                                ...
                                <span :class="getLevelClass(log.level)" class="w-16 shrink-0">{{ log.level }}</span>
                                <span class="text-blue-500 dark:text-blue-400 w-32 truncate shrink-0" :title="log.task_name">{{ log.task_name }}</span>
