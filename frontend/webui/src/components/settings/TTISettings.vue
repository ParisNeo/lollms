...
<template>
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <h2 class="text-xl font-bold leading-6 text-gray-900 dark:text-white">Image Generation (TTI) Parameters</h2>
        ...
        <div class="border-t border-gray-200 dark:border-gray-700">
            <form @submit.prevent="handleSave" class="p-4 sm:p-6 space-y-6">
                    ...
                    <div class="relative">
                        <button ref="ttiTriggerRef" @click="isTtiMenuOpen = !isTtiMenuOpen" type="button" class="toolbox-select truncate w-full flex items-center justify-between">
                            <div class="flex items-center space-x-3 truncate">
                                ...
                                <span v-else class="w-8 h-8 shrink-0 text-gray-500 dark:text-gray-400 flex items-center justify-center bg-gray-200 dark:bg-gray-700 rounded-md"><IconPhoto class="w-5 h-5" /></span>
                                        ...
                                        <div v-for="group in filteredAvailableTtiModels" :key="group.label">
                                            ...
                                            <button v-for="item in group.items" :key="item.id" @click="selectTtiModel(item.id)" class="menu-item-button" :class="{'selected': activeTtiModel === item.id}">
                                                <div class="flex items-center space-x-3 truncate">
                                                    ...
                                                    <IconPhoto v-else class="w-6 h-6 text-gray-500 dark:text-gray-400 shrink-0" />
                                                    <div class="truncate text-left"><p class="font-medium truncate text-sm">{{ item.name }}</p></div>
                                                </div>
                                                <IconCheckCircle v-if="activeTtiModel === item.id" class="w-5 h-5 text-blue-600 dark:text-blue-400 shrink-0" />
                                            </button>
                                        </div>
                                    </div>
                                </div>
                ...
                <div v-if="activeTtiModel && selectedModelDetails && allowOverrides && modelConfigurableParameters.length > 0" class="p-4 border rounded-lg dark:border-gray-700 space-y-4">
