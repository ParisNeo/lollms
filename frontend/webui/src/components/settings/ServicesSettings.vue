...
async function handleGenerateSecret(item, type) {
    const confirmed = await uiStore.showConfirmation({
        ...
        message: `This will generate a new SSO secret for '${item.name}'. Your existing application using the old secret will no longer be able to authenticate. This action is irreversible.`,
        confirmText: 'Generate Secret'
    });
    if (confirmed) {
        const isApp = type === 'apps';
        const secret = await (isApp ? dataStore.generateAppSsoSecret(item.id) : dataStore.generateMcpSsoSecret(item.id));
        if (secret) {
