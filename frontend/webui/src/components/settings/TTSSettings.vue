...
// Dropdown state
const isTtsMenuOpen = ref(false);
const ttsTriggerRef = ref(null);
const ttsFloatingRef = ref(null);
const ttsModelSearchTerm = ref('');

const { floatingStyles: ttsFloatingStyles } = useFloating(ttsTriggerRef, ttsFloatingRef, {
...
const vOnClickOutside = {
  mounted: (el, binding) => {
    el.clickOutsideEvent = event => {
      const triggerEl = ttsTriggerRef.value;
...
watch(isTtsMenuOpen, (isOpen) => {
    if (isOpen) {
        nextTick(() => {
            const menuElement = ttsFloatingRef.value;
            if (menuElement) {
...
const selectedModelDetails = computed(() => {
    ...
    return availableTtsModels.value.find(m => m.id === activeTtsModel.value) || null;
});
...
const userOverrides = computed(() => {
    if (!user.value || !activeTtsModel.value || !user.value.tts_models_config) return {};
    return user.value.tts_models_config[activeTtsModel.value] || {};
});
...
const filteredAvailableTtsModels = computed(() => {
    if (!ttsModelSearchTerm.value) return availableTtsModelsGrouped.value;
    const term = ttsModelSearchTerm.value.toLowerCase();
    const result = [];
    for (const group of availableTtsModelsGrouped.value) {
        if (group.items) {
