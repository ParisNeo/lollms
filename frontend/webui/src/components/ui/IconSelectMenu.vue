    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          ...
          <ul class="grow overflow-y-auto p-1 max-h-80">
             ...
             <template v-else>
                ...
                <div class="flex items-center space-x-3">
                    <div class="h-8 w-8 rounded-md bg-gray-200 dark:bg-gray-600 shrink-0 flex items-center justify-center">
                ...
                <div v-if="item.isGroup" class="pt-2">
                    ...
                    <li v-for="subItem in item.items" :key="subItem.id" @click="selectOption(subItem.id)" class="menu-item" :class="{'is-selected': subItem.id === modelValue}">
                        <div class="flex items-center space-x-3 grow min-w-0">
                            <img v-if="subItem.icon_base64" :src="subItem.icon_base64" class="h-8 w-8 rounded-md object-cover shrink-0"/>
                            <div v-else class="h-8 w-8 rounded-md bg-gray-200 dark:bg-gray-600 shrink-0 flex items-center justify-center text-gray-400">
                ...
                <li v-else @click="selectOption(item.id)" class="menu-item" :class="{'is-selected': item.id === modelValue}">
                    <div class="flex items-center space-x-3 grow min-w-0">
                    <img v-if="item.icon_base64" :src="item.icon_base64" class="h-8 w-8 rounded-md object-cover shrink-0"/>
                    <div v-else class="h-8 w-8 rounded-md bg-gray-200 dark:bg-gray-600 shrink-0 flex items-center justify-center text-gray-400">
                        <slot name="item-icon-default"></slot>
                    </div>
                    <div class="min-w-0">
