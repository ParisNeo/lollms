...
const scratchpadTokenCount = ref(0); // NEW
const isTokenizingInput = ref(false);
const isTokenizingScratchpad = ref(false); // NEW
let tokenizeInputDebounceTimer = null;
let tokenizeScratchpadDebounceTimer = null; // NEW
...
const totalCurrentTokens = computed(() => discussionTokens.value + inputTokenCount.value + scratchpadTokenCount.value); // UPDATED
...
// NEW
const scratchpadTokensPercentage = computed(() => {
    if (maxTokens.value <= 0) return 0;
    return (scratchpadTokenCount.value / maxTokens.value) * 100;
});
...
watch(() => props.scratchpadContent, (newValue) => {
    clearTimeout(tokenizeScratchpadDebounceTimer);
    if (!newValue.trim()) {
        scratchpadTokenCount.value = 0;
    } else if (showContextBar.value) {
        tokenizeScratchpadDebounceTimer = setTimeout(() => {
            fetchTokenCount(newValue, scratchpadTokenCount, isTokenizingScratchpad);
        }, 500);
    }
}, { immediate: true });
...
watch(activeDiscussion, () => {
    ...
    clearTimeout(tokenizeScratchpadDebounceTimer);
    inputTokenCount.value = 0;
    scratchpadTokenCount.value = 0; // Reset on discussion change
    // Trigger re-tokenization of new scratchpad content
    if (props.scratchpadContent) {
        fetchTokenCount(props.scratchpadContent, scratchpadTokenCount, isTokenizingScratchpad);
    }
});
            ...
            <div v-if="showContextBar" class="px-1 pb-2">
                <div class="flex justify-between items-center mb-1 text-xs text-gray-600 dark:text-gray-400 font-mono">
                    <div class="flex items-center gap-1.5">
                        ...
                        <span>Context: {{ discussionTokens.toLocaleString() }} + {{ inputTokenCount.toLocaleString() }} + {{ scratchpadTokenCount.toLocaleString() }}</span>
                    </div>
                ...
                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 relative overflow-hidden">
                    ...
                    ></div>
                    <div 
                        class="absolute top-0 h-full transition-all duration-300 opacity-75"
                        :class="progressColorClass" 
                        :style="{ left: `${Math.min(discussionTokensPercentage, 100)}%`, width: `${Math.min(scratchpadTokensPercentage, 100)}%` }"
                    ></div>
                    <div 
                        class="absolute top-0 h-full transition-all duration-300"
                        :class="progressColorClass" 
                        :style="{ left: `${Math.min(discussionTokensPercentage + scratchpadTokensPercentage, 100)}%`, width: `${Math.min(inputTokensPercentage, 100)}%` }"
                    ></div>
                </div>
            </div>
