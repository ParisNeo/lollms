...
watch(selectedStore, async (newStore) => {
    if (newStore) {
        ...
        storeFiles.value = [];
        storeVectorizersInStore.value = [];
        storeVectorizersAllPossible.value = [];
...
async function handleManage(store) {
    ...
    try {
        ...
        // Intelligent default vectorizer selection for UPLOADS
        selectedVectorizer.value = user.value?.safe_store_vectorizer || '';
        // If the user's default isn't in the store, add it to the list for selection
        ...
        // Default for RE-VECTORIZE is the first one in the store
