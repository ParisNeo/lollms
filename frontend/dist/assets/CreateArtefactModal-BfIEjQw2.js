...
\\end{document}`},{label:`Equation`,code:`\\begin{equation}
    e=mc^2
\\end{equation}`}],svg:[{label:`Circle`,code:`<circle cx="50" cy="50" r="40" stroke="black" stroke-width="3" fill="red" />`},{label:`Rect`,code:`<rect width="300" height="100" style="fill:rgb(0,0,255);stroke-width:3;stroke:rgb(0,0,0)" />`}]},J=S(()=>q[G.value]||[]);h(()=>L.isModalOpen(`createArtefact`),e=>{e&&(H.value=`Untitled Document.md`,U.value=``,G.value=`markdown`)}),h(G,e=>{let t=K.find(t=>t.id===e);if(t&&H.value.includes(`.`)){let e=H.value.split(`.`)[0];H.value=e+t.ext}});function Y(e){U.value.trim()?U.value+=`

`+e:U.value=e}async function X(){if(!V.value&&!B.value){L.addNotification(`No discussion selected.`,`error`);return}if(!H.value.trim()){L.addNotification(`Title is required.`,`warning`);return}W.value=!0;try{V.value?await R.saveRawArtefactToLibrary({title:H.value.trim(),content:U.value,artefactType:G.value===`markdown`?`document`:`code`}):await R.createManualArtefact({discussionId:B.value,title:H.value.trim(),content:U.value,imagesB64:[]}),L.closeModal(`createArtefact`)}catch(e){console.error(`Failed to save artefact:`,e)}finally{W.value=!1}}return(l,h)=>(g(),s(C,{modalName:`createArtefact`,title:`Create New Document`,maxWidthClass:`max-w-4xl`},{body:d(()=>[e(`div`,w,[e(`div`,T,[e(`div`,E,[h[4]||=e(`label`,{for:`artefact-title`,class:`label`},`Document Title`,-1),e(`div`,D,[e(`div`,O,[y(_,{class:`h-4 w-4 text-gray-400`})]),f(e(`input`,{id:`artefact-title`,"onUpdate:modelValue":h[0]||=e=>H.value=e,type:`text`,class:`input-field pl-10`,placeholder:`e.g. My Notes.md`,required:``},null,512),[[a,H.value]])])]),e(`div`,null,[h[5]||=e(`label`,{for:`artefact-type`,class:`label`},`Language / Format`,-1),f(e(`select`,{"onUpdate:modelValue":h[1]||=e=>G.value=e,class:`input-field mt-1`},[(g(),o(r,null,p(K,n=>e(`option`,{key:n.id,value:n.id},t(n.name)+` (`+t(n.ext)+`) `,9,k)),64))],512),[[m,G.value]])])]),J.value.length>0?(g(),o(`div`,A,[h[6]||=e(`span`,{class:`text-[10px] font-black uppercase text-gray-400 self-center mr-2`},`Quick Snippets:`,-1),(g(!0),o(r,null,p(J.value,e=>(g(),o(`button`,{key:e.label,onClick:t=>Y(e.code),class:`px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-blue-500 hover:text-white text-[10px] font-bold transition-all border dark:border-gray-700`},` + `+t(e.label),9,j))),128))])):n(``,!0),e(`div`,M,[h[7]||=e(`label`,{class:`label mb-1`},`Content`,-1),e(`div`,N,[y(c,{modelValue:U.value,"onUpdate:modelValue":h[2]||=e=>U.value=e,class:`h-full absolute inset-0`,language:G.value,allowedModes:`both`,placeholder:`Start typing or use a snippet above...`},null,8,[`modelValue`,`language`])])])])]),footer:d(()=>[e(`div`,P,[e(`button`,{onClick:h[3]||=e=>u(L).closeModal(`createArtefact`),type:`button`,class:`btn btn-secondary`},`Cancel`),e(`button`,{onClick:X,type:`button`,class:`btn btn-primary px-10`,disabled:W.value||!H.value.trim()},[W.value?(g(),s(x,{key:0,class:`w-4 h-4 mr-2 animate-spin`})):n(``,!0),i(` `+t(V.value?`Create & Save`:`Create & Load`),1)],8,F)])]),_:1}))}};export{I as default};