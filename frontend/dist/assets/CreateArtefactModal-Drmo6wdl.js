...
`+e:U.value=e}async function X(){if(!V.value&&!B.value){L.addNotification(`No discussion selected.`,`error`);return}if(!H.value.trim()){L.addNotification(`Title is required.`,`warning`);return}W.value=!0;try{V.value?await R.saveRawArtefactToLibrary({title:H.value.trim(),content:U.value,artefactType:G.value===`markdown`?`document`:`code`}):await R.createManualArtefact({discussionId:B.value,title:H.value.trim(),content:U.value,imagesB64:[]}),L.closeModal(`createArtefact`)}catch(e){console.error(`Failed to save artefact:`,e)}finally{W.value=!1}}return(r,a)=>(c(),b(C,{modalName:`createArtefact`,title:`Create New Document`,maxWidthClass:`max-w-4xl`},{body:m(()=>[v(`div`,w,[v(`div`,T,[v(`div`,E,[a[4]||=v(`label`,{for:`artefact-title`,class:`label`},`Document Title`,-1),v(`div`,D,[v(`div`,O,[y(t,{class:`h-4 w-4 text-gray-400`})]),i(v(`input`,{id:`artefact-title`,"onUpdate:modelValue":a[0]||=e=>H.value=e,type:`text`,class:`input-field pl-10`,placeholder:`e.g. My Notes.md`,required:``},null,512),[[o,H.value]])])]),v(`div`,null,[a[5]||=v(`label`,{for:`artefact-type`,class:`label`},`Language / Format`,-1),i(v(`select`,{"onUpdate:modelValue":a[1]||=e=>G.value=e,class:`input-field mt-1`},[(c(),g(_,null,x(K,e=>v(`option`,{key:e.id,value:e.id},l(e.name)+` (`+l(e.ext)+`) `,9,k)),64))],512),[[u,G.value]])])]),J.value.length>0?(c(),g(`div`,A,[a[6]||=v(`span`,{class:`text-[10px] font-black uppercase text-gray-400 self-center mr-2`},`Quick Snippets:`,-1),(c(!0),g(_,null,x(J.value,e=>(c(),g(`button`,{key:e.label,onClick:t=>Y(e.code),class:`px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-blue-500 hover:text-white text-[10px] font-bold transition-all border dark:border-gray-700`},` + `+l(e.label),9,j))),128))])):f(``,!0),v(`div`,M,[a[7]||=v(`label`,{class:`label mb-1`},`Content`,-1),v(`div`,N,[y(n,{modelValue:U.value,"onUpdate:modelValue":a[2]||=e=>U.value=e,class:`h-full absolute inset-0`,language:G.value,allowedModes:`both`,placeholder:`Start typing or use a snippet above...`},null,8,[`modelValue`,`language`])])])])]),footer:m(()=>[v(`div`,P,[v(`button`,{onClick:a[3]||=t=>e(L).closeModal(`createArtefact`),type:`button`,class:`btn btn-secondary`},`Cancel`),v(`button`,{onClick:X,type:`button`,class:`btn btn-primary px-10`,disabled:W.value||!H.value.trim()},[W.value?(c(),b(s,{key:0,class:`w-4 h-4 mr-2 animate-spin`})):f(``,!0),p(` `+l(V.value?`Create & Save`:`Create & Load`),1)],8,F)])]),_:1}))}};export{I as default};