...
\\end{document}`},{label:`Equation`,code:`\\begin{equation}
    e=mc^2
\\end{equation}`}],svg:[{label:`Circle`,code:`<circle cx="50" cy="50" r="40" stroke="black" stroke-width="3" fill="red" />`},{label:`Rect`,code:`<rect width="300" height="100" style="fill:rgb(0,0,255);stroke-width:3;stroke:rgb(0,0,0)" />`}]},J=e(()=>q[G.value]||[]);y(()=>L.isModalOpen(`createArtefact`),e=>{e&&(H.value=`Untitled Document.md`,U.value=``,G.value=`markdown`)}),y(G,e=>{let t=K.find(t=>t.id===e);if(t&&H.value.includes(`.`)){let e=H.value.split(`.`)[0];H.value=e+t.ext}});function Y(e){U.value.trim()?U.value+=`

`+e:U.value=e}async function X(){if(!V.value&&!B.value){L.addNotification(`No discussion selected.`,`error`);return}if(!H.value.trim()){L.addNotification(`Title is required.`,`warning`);return}W.value=!0;try{V.value?await R.saveRawArtefactToLibrary({title:H.value.trim(),content:U.value,artefactType:G.value===`markdown`?`document`:`code`}):await R.createManualArtefact({discussionId:B.value,title:H.value.trim(),content:U.value,imagesB64:[]}),L.closeModal(`createArtefact`)}catch(e){console.error(`Failed to save artefact:`,e)}finally{W.value=!1}}return(e,m)=>(_(),t(C,{modalName:`createArtefact`,title:`Create New Document`,maxWidthClass:`max-w-4xl`},{body:p(()=>[l(`div`,w,[l(`div`,T,[l(`div`,E,[m[4]||=l(`label`,{for:`artefact-title`,class:`label`},`Document Title`,-1),l(`div`,D,[l(`div`,O,[n(f,{class:`h-4 w-4 text-gray-400`})]),h(l(`input`,{id:`artefact-title`,"onUpdate:modelValue":m[0]||=e=>H.value=e,type:`text`,class:`input-field pl-10`,placeholder:`e.g. My Notes.md`,required:``},null,512),[[a,H.value]])])]),l(`div`,null,[m[5]||=l(`label`,{for:`artefact-type`,class:`label`},`Language / Format`,-1),h(l(`select`,{"onUpdate:modelValue":m[1]||=e=>G.value=e,class:`input-field mt-1`},[(_(),u(r,null,d(K,e=>l(`option`,{key:e.id,value:e.id},x(e.name)+` (`+x(e.ext)+`) `,9,k)),64))],512),[[i,G.value]])])]),J.value.length>0?(_(),u(`div`,A,[m[6]||=l(`span`,{class:`text-[10px] font-black uppercase text-gray-400 self-center mr-2`},`Quick Snippets:`,-1),(_(!0),u(r,null,d(J.value,e=>(_(),u(`button`,{key:e.label,onClick:t=>Y(e.code),class:`px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-blue-500 hover:text-white text-[10px] font-bold transition-all border dark:border-gray-700`},` + `+x(e.label),9,j))),128))])):c(``,!0),l(`div`,M,[m[7]||=l(`label`,{class:`label mb-1`},`Content`,-1),l(`div`,N,[n(o,{modelValue:U.value,"onUpdate:modelValue":m[2]||=e=>U.value=e,class:`h-full absolute inset-0`,language:G.value,allowedModes:`both`,placeholder:`Start typing or use a snippet above...`},null,8,[`modelValue`,`language`])])])])]),footer:p(()=>[l(`div`,P,[l(`button`,{onClick:m[3]||=e=>S(L).closeModal(`createArtefact`),type:`button`,class:`btn btn-secondary`},`Cancel`),l(`button`,{onClick:X,type:`button`,class:`btn btn-primary px-10`,disabled:W.value||!H.value.trim()},[W.value?(_(),t(s,{key:0,class:`w-4 h-4 mr-2 animate-spin`})):c(``,!0),g(` `+x(V.value?`Create & Save`:`Create & Load`),1)],8,F)])]),_:1}))}};export{I as default};