...
`+e:U.value=e}async function X(){if(!V.value&&!B.value){L.addNotification(`No discussion selected.`,`error`);return}if(!H.value.trim()){L.addNotification(`Title is required.`,`warning`);return}W.value=!0;try{V.value?await R.saveRawArtefactToLibrary({title:H.value.trim(),content:U.value,artefactType:G.value===`markdown`?`document`:`code`}):await R.createManualArtefact({discussionId:B.value,title:H.value.trim(),content:U.value,imagesB64:[]}),L.closeModal(`createArtefact`)}catch(e){console.error(`Failed to save artefact:`,e)}finally{W.value=!1}}return(i,m)=>(h(),r(C,{modalName:`createArtefact`,title:`Create New Document`,maxWidthClass:`max-w-4xl`},{body:y(()=>[g(`div`,w,[g(`div`,T,[g(`div`,E,[m[4]||=g(`label`,{for:`artefact-title`,class:`label`},`Document Title`,-1),g(`div`,D,[g(`div`,O,[p(d,{class:`h-4 w-4 text-gray-400`})]),x(g(`input`,{id:`artefact-title`,"onUpdate:modelValue":m[0]||=e=>H.value=e,type:`text`,class:`input-field pl-10`,placeholder:`e.g. My Notes.md`,required:``},null,512),[[S,H.value]])])]),g(`div`,null,[m[5]||=g(`label`,{for:`artefact-type`,class:`label`},`Language / Format`,-1),x(g(`select`,{"onUpdate:modelValue":m[1]||=e=>G.value=e,class:`input-field mt-1`},[(h(),l(c,null,f(K,e=>g(`option`,{key:e.id,value:e.id},t(e.name)+` (`+t(e.ext)+`) `,9,k)),64))],512),[[s,G.value]])])]),J.value.length>0?(h(),l(`div`,A,[m[6]||=g(`span`,{class:`text-[10px] font-black uppercase text-gray-400 self-center mr-2`},`Quick Snippets:`,-1),(h(!0),l(c,null,f(J.value,e=>(h(),l(`button`,{key:e.label,onClick:t=>Y(e.code),class:`px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-blue-500 hover:text-white text-[10px] font-bold transition-all border dark:border-gray-700`},` + `+t(e.label),9,j))),128))])):u(``,!0),g(`div`,M,[m[7]||=g(`label`,{class:`label mb-1`},`Content`,-1),g(`div`,N,[p(o,{modelValue:U.value,"onUpdate:modelValue":m[2]||=e=>U.value=e,class:`h-full absolute inset-0`,language:G.value,allowedModes:`both`,placeholder:`Start typing or use a snippet above...`},null,8,[`modelValue`,`language`])])])])]),footer:y(()=>[g(`div`,P,[g(`button`,{onClick:m[3]||=e=>n(L).closeModal(`createArtefact`),type:`button`,class:`btn btn-secondary`},`Cancel`),g(`button`,{onClick:X,type:`button`,class:`btn btn-primary px-10`,disabled:W.value||!H.value.trim()},[W.value?(h(),r(a,{key:0,class:`w-4 h-4 mr-2 animate-spin`})):u(``,!0),e(` `+t(V.value?`Create & Save`:`Create & Load`),1)],8,F)])]),_:1}))}};export{I as default};