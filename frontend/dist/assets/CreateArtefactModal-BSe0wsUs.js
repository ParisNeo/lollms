...
\\end{document}`},{label:`Equation`,code:`\\begin{equation}
    e=mc^2
\\end{equation}`}],svg:[{label:`Circle`,code:`<circle cx="50" cy="50" r="40" stroke="black" stroke-width="3" fill="red" />`},{label:`Rect`,code:`<rect width="300" height="100" style="fill:rgb(0,0,255);stroke-width:3;stroke:rgb(0,0,0)" />`}]},J=a(()=>q[G.value]||[]);h(()=>L.isModalOpen(`createArtefact`),e=>{e&&(H.value=`Untitled Document.md`,U.value=``,G.value=`markdown`)}),h(G,e=>{let t=K.find(t=>t.id===e);t&&H.value.includes(`.`)&&(H.value=H.value.split(`.`)[0]+t.ext)});function Y(e){U.value.trim()?U.value+=`

`+e:U.value=e}async function X(){if(!V.value&&!B.value){L.addNotification(`No discussion selected.`,`error`);return}if(!H.value.trim()){L.addNotification(`Title is required.`,`warning`);return}W.value=!0;try{V.value?await R.saveRawArtefactToLibrary({title:H.value.trim(),content:U.value,artefactType:G.value===`markdown`?`document`:`code`}):await R.createManualArtefact({discussionId:B.value,title:H.value.trim(),content:U.value,imagesB64:[]}),L.closeModal(`createArtefact`)}catch(e){console.error(`Failed to save artefact:`,e)}finally{W.value=!1}}return(a,d)=>(_(),i(C,{modalName:`createArtefact`,title:`Create New Document`,maxWidthClass:`max-w-4xl`},{body:y(()=>[v(`div`,w,[v(`div`,T,[v(`div`,E,[d[4]||=v(`label`,{for:`artefact-title`,class:`label`},`Document Title`,-1),v(`div`,D,[v(`div`,O,[p(s,{class:`h-4 w-4 text-gray-400`})]),x(v(`input`,{id:`artefact-title`,"onUpdate:modelValue":d[0]||=e=>H.value=e,type:`text`,class:`input-field pl-10`,placeholder:`e.g. My Notes.md`,required:``},null,512),[[S,H.value]])])]),v(`div`,null,[d[5]||=v(`label`,{for:`artefact-type`,class:`label`},`Language / Format`,-1),x(v(`select`,{"onUpdate:modelValue":d[1]||=e=>G.value=e,class:`input-field mt-1`},[(_(),l(c,null,f(K,e=>v(`option`,{key:e.id,value:e.id},n(e.name)+` (`+n(e.ext)+`) `,9,k)),64))],512),[[o,G.value]])])]),J.value.length>0?(_(),l(`div`,A,[d[6]||=v(`span`,{class:`text-[10px] font-black uppercase text-gray-400 self-center mr-2`},`Quick Snippets:`,-1),(_(!0),l(c,null,f(J.value,e=>(_(),l(`button`,{key:e.label,onClick:t=>Y(e.code),class:`px-2 py-1 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-blue-500 hover:text-white text-[10px] font-bold transition-all border dark:border-gray-700`},` + `+n(e.label),9,j))),128))])):u(``,!0),v(`div`,M,[d[7]||=v(`label`,{class:`label mb-1`},`Content`,-1),v(`div`,N,[p(m,{modelValue:U.value,"onUpdate:modelValue":d[2]||=e=>U.value=e,class:`h-full absolute inset-0`,language:G.value,allowedModes:`both`,placeholder:`Start typing or use a snippet above...`},null,8,[`modelValue`,`language`])])])])]),footer:y(()=>[v(`div`,P,[v(`button`,{onClick:d[3]||=e=>r(L).closeModal(`createArtefact`),type:`button`,class:`btn btn-secondary`},`Cancel`),v(`button`,{onClick:X,type:`button`,class:`btn btn-primary px-10`,disabled:W.value||!H.value.trim()},[W.value?(_(),i(t,{key:0,class:`w-4 h-4 mr-2 animate-spin`})):u(``,!0),e(` `+n(V.value?`Create & Save`:`Create & Load`),1)],8,F)])]),_:1}))}};export{I as default};